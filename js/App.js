...
//         .join("");
//       window.hashedfile = web3.utils.soliditySha3("0x" + hexString);
...
function print_verification_info(result, is_verified) {
  ...
  document.getElementById("student-document").data = "./files/notvalid.svg";
  $("#loader").hide();
...
function printUploadInfo(result) {
  ...
  // Set download link to actual IPFS document

// Set download link to IPFS document
document.getElementById("download-link").href = `https://ipfs.io/ipfs/${window.currentCID}`;
...
async function sendHash() {
  ...
  window.currentCID = CID; // Save globally
  if (window.hashedfile.length > 4) {
    await window.contract.methods
      .addDocHash(window.hashedfile, CID)
      .send({ from: window.userAddress, gas: 200000 })
...
async function get_ChainID() {
  ...
  if (network) {
    document.getElementById("network").innerHTML =
      `<i class="text-info fa-solid fa-circle-nodes mx-2"></i>${window.chainID}`;
  }
}
...
//   });
//   if (!window.hashedfile) return;
//   let ipfsUrl = `https://ipfs.io/ipfs/${window.currentCID}`;
// qrcode.makeCode(ipfsUrl);
// document.getElementById("verfiy").href = 
//   window.location.protocol + "//" + window.location.host + 
//   `/verify.html?hash=${window.hashedfile}`;
...
function generateQRCode() {
  ...
  // QR code points to the IPFS document URL
  qrcode.makeCode(ipfsUrl);
  ...
  // ✅ Download link points to the actual IPFS document, opens in new tab
  const downloadLink = document.getElementById("download-link");
  downloadLink.href = ipfsUrl;
  downloadLink.setAttribute("target", "_blank");
  downloadLink.removeAttribute("download"); // Don't force-download, let IPFS serve it
}
